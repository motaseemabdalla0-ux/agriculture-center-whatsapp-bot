...
// مسار Broadcast فقط (مش تحقق البوت): رقم سعودي صحيح يمشي زي ما هو، وأي رقم دولي غير سعودي
// بيتقبل بس لو مكتوب بمقدمة دولية صريحة (+ أو 00) وطوله E.164 معقول (8-15 رقم، أول رقم 1-9).
// من غير المقدمة الصريحة مفيش تخمين لكود دولة - الرقم بيترفض.
function normalizeForBroadcast(raw) {
  const saudi = normalizeSaudiPhone(raw);
  if (isValidSaudiPhone(saudi)) return { phone: saudi, international: false };
  const text = toWesternDigits(String(raw || "")).trim();
  if (!/^(\+|00)/.test(text)) return null;
  const digits = text.replace(/[^\d]/g, "").replace(/^00/, "");
  return /^[1-9]\d{7,14}$/.test(digits) ? { phone: digits, international: true } : null;
}
...
function analyze(buffer) {
  ...
  const preview = { total: rawRows.length, valid: 0, invalidPhones: 0, duplicates: 0, missingNames: 0, missingMessages: 0, arabicNames: 0, internationalNumbers: 0, ready: 0 };
  const seen = new Set();
  const ready = [];

  rawRows.forEach((r) => {
    const name = (r.name || "").trim();
    const norm = normalizeForBroadcast(r.phone);
    if (!norm) {
    ...
    seen.add(phone);
    if (norm.international) preview.internationalNumbers++;
