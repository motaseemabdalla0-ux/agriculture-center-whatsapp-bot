...
const deliveryStore = require("./deliveryStore");
...
async function buildPendingItemsReport(client = null) {
  ...
  const openDeliveries = deliveryStore.listOpenRequests();

  return { openTickets, ticketsByCategory, handedOff, pendingDocuments, openDeliveries };
}
...
function formatPendingItemsReport(data) {
  ...
  lines.push("");
  lines.push(`🚚 طلبات توصيل البطاقة المفتوحة (${data.openDeliveries.length}):`);
  data.openDeliveries.slice(0, MAX_LISTED).forEach((r) =>
    lines.push(`    • ${r.id} - ${r.name || "بدون اسم"} - ${formatContactLine(r.phone)} - ${deliveryStore.REGION_BY_KEY[r.regionKey].ar}${r.forwarded ? "" : " ⚠️ لم يُرسل للمجموعة"}`)
  );
  if (data.openDeliveries.length > MAX_LISTED) lines.push(`    ... و${data.openDeliveries.length - MAX_LISTED} إضافية`);
  if (data.openDeliveries.length === 0) lines.push("    لا يوجد.");
