...
// مش بديل عن التأخير العشوائي بين كل رسالة - ده منفصل وموجود في كل Runner، والاتنين بيشتغلوا
// مع بعض. القيم قابلة للتخصيص من متغيرات البيئة (.env):
//   RATE_LIMIT_HOURLY (افتراضي 15), RATE_LIMIT_DAILY (افتراضي 100),
//   RATE_LIMIT_COOLDOWN_MS (افتراضي 0 = بدون تهدئة إضافية غير التأخير العشوائي الموجود أصلًا)
//
// الحدود اتخفّضت من 50/500 إلى 15/100 بعد ما واتساب قيّد حساب البوت فعليًا (رسالة "حسابك مقيّد
// حاليًا" - اكتشاف نمط رسائل جماعية/آلية) أثناء حملة حقيقية. السبب مش سرعة الإرسال بس - هو حجم
// ونمط الإرسال الجماعي عمومًا عبر أداة غير رسمية (whatsapp-web.js) - لكن تقليل الحدود بيقلل شدة
// النمط المكتشف ومعدّل التكرار اليومي، وهو أهم إجراء تقني ممكن دون تغيير الأداة نفسها.
const STORE_FILE = path.join(__dirname, "..", "rate_limit_state.json");

function getConfig() {
  return {
    hourly: parseInt(process.env.RATE_LIMIT_HOURLY || "15", 10),
    daily: parseInt(process.env.RATE_LIMIT_DAILY || "100", 10),
