...
// "blocked_previous_contact" | "blocked_review_required" | "duplicate" | "rate_limited" |
// "not_on_whatsapp" | "failed" | "delivery_uncertain"
// (delivery_uncertain: الإرسال الفعلي لواتساب نجح بالتأكيد، لكن حفظ الحالة بعده فشل - ممنوع
// اعتباره فشل عادي قابل لإعادة المحاولة، عشان منبعتش نفس الرسالة مرتين لمزارع استلمها فعلًا)
async function safeFarmerSend(client, options) {
  ...
  // 1.5) Farmer Registry + Communication History - بس لحملات purpose الجديدة (custom campaigns).
  // بيمنع إعادة إرسال نفس الغرض لمزارع سبق التواصل معاه بيه فعلًا (بغض النظر عن نص الرسالة)،
  // وبيمنع الإرسال لو فيه تعارض واضح بين السجل وحالة البوابة (محتاج مراجعة يدوية). مفيش أي تأثير
  // على الحملات القديمة بـcampaignType (الشرط purpose بيمنع دخول أي حملة قديمة هنا خالص)
  if (purpose && messageSource === "CAMPAIGN") {
    const historyCheck = contactHistoryGuard.checkContactHistoryBlock({ phone: normalizedPhone, purpose, state });
    if (historyCheck.blocked) {
      auditLog.record({ ...auditBase, farmerState: state, decision: historyCheck.status.toUpperCase(), reason: historyCheck.reason });
      return { status: historyCheck.status, state, reason: historyCheck.reason };
    }
  }
  ...
  // فحص حاسم: فصل صريح بين فشل الإرسال الفعلي نفسه (client.sendMessage) وأي فشل لاحق في خطوات
  // الحفظ (confirmSent/markSent). حادثة حقيقية أثبتت ضرورة الفصل ده: حملة إنتاج على 3,866 مزارع -
  // رسائل فعلية اتبعتت بنجاح لواتساب، لكن EPERM عابر وقت حفظ send_fingerprints.json خلّى الكود
  // القديم (try واحد يغلّف الاتنين) يصنّفها "failed" بالغلط - ده كان هيسمح بإعادة إرسال رسائل
  // فعلًا وصلت للمزارع (تكرار حقيقي)! الحل: try منفصل لكل مرحلة.
  try {
    await sendWithRetry(client, numberId._serialized, message);
  } catch (sendErr) {
    // الإرسال الفعلي نفسه فشل - مفيش أي شك، الرسالة ما وصلتش، آمن نحرّر الحجز ونسمح بمحاولة لاحقة
    sendFingerprint.release(fingerprint);
    auditLog.record({ ...auditBase, farmerState: state, messageFingerprint: fingerprint, decision: "FAILED", reason: sendErr.message });
    return { status: "failed", reason: sendErr.message };
  }

  // الإرسال الفعلي نجح بالتأكيد من هنا فصاعدًا - أي خطأ جاي دلوقتي هو مشكلة حفظ بس، مش فشل
  // إرسال. ممنوع نرجّع "failed" هنا (ده كان بيسمح بإعادة إرسال حقيقي مكرر) - بنرجّع حالة مختلفة
  // تمامًا (delivery_uncertain) عشان الجهة المستدعية (campaignRunner/personalizedRunner) تعرف
  // متعتبرش الصف ده قابل لإعادة الإرسال التلقائي أبدًا، حتى لو معندهاش تأكيد كتابة فعلي
  try {
    ...
    console.log(`🚨 نجح إرسال واتساب فعليًا لكن فشل حفظ الحالة بعده (${persistErr.message}) - الصف اتصنّف DELIVERY_UNCERTAIN ومش هيتكرر إرساله تلقائيًا`);
    try {
      auditLog.record({ ...auditBase, farmerState: state, messageFingerprint: fingerprint, decision: "DELIVERY_UNCERTAIN", reason: persistErr.message });
    } catch {
      // حتى تسجيل الـAudit نفسه فشل - مش سبب نغيّر القرار، الأهم إننا منرجعش "sent" ولا "failed"
    }
    return { status: "delivery_uncertain", reason: persistErr.message, state };
  }
}
