...
// بيرجع كل رقم بعت مستند/صورة (document_received) ومفيش أي حدث تاني اتسجّل ليه بعد كده -
// معناها لسه محتاج مراجعة/رد بشري. أي رسالة تانية بعدها (حتى لو رد تلقائي بسيط زي اختيار من
// القائمة) بتشيله من القائمة دي، فمش دليل قاطع 100% إن حد راجعه فعليًا، بس أفضل تقريب متاح
// من غير ربط مباشر بـticketStore/sessionStore (اللي أصلاً مش بيسجّلوا في activity_log.csv)
function getPendingDocumentSubmissions() {
  if (!fs.existsSync(ACTIVITY_LOG)) return [];
  const lastByPhone = new Map();
  const lines = fs.readFileSync(ACTIVITY_LOG, "utf8").split(/\r?\n/).filter(Boolean);
  for (const line of lines) {
  ...
  return Array.from(lastByPhone.values()).filter((e) => e.type === "document_received");
}
