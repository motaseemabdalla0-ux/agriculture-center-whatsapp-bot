...
const { isArabicName, templateUsesName } = require("./arabicNameGuard");
...
async function runPersonalizedBroadcast(client, options) {
  const { csvPath, columns, template, logFile, logLabel, campaignType, messageSource = "CAMPAIGN", onSent } = options;
  const summary = { total: 0, sent: 0, failed: 0, notOnWhatsapp: 0, invalidPhone: 0, skippedDuplicate: 0, blockedNonArabicName: 0, deliveryUncertain: 0 };
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // قاعدة أمان الاسم العربي (إلزامية): بس لو القالب فعلًا بيستخدم {name}. الأولوية: اسم عربي
    // موثوق من السجل الدائم -> الاسم نفسه لو عربي فعلًا. ممنوع أي تعريب تلقائي - لو الاسم في
    // الملف مش عربي ومفيش اسم عربي موثوق محفوظ من قبل، الرسالة دي بتتمنع تمامًا (مش بتتبعت
    // بالاسم الإنجليزي أبدًا) وبتتسجّل في Audit Log بسبب BLOCKED_NON_ARABIC_NAME
    const usesName = templateUsesName(template);
    let resolvedName = row.name;
    if (usesName && !isArabicName(row.name)) {
      const trusted = farmerRegistry.resolveTrustedArabicName(phone, row.name);
      if (!trusted) {
        console.log("🚫 ممنوع - الاسم غير عربي ومفيش اسم عربي موثوق محفوظ (BLOCKED_NON_ARABIC_NAME)");
        log.push(`${label} (${phone}) -> ممنوع (اسم غير عربي)`);
        auditLog.record({
          normalizedPhone: normalizeSaudiPhone(phone),
          campaignType,
          decision: "BLOCKED_NON_ARABIC_NAME",
          reason: "BLOCKED_NON_ARABIC_NAME",
        });
        summary.blockedNonArabicName++;
        if (i < rows.length - 1 && process.env.CAMPAIGN_TEST_FAST !== "true") await sleep(randomDelayMs());
        continue;
      }
      resolvedName = trusted;
    }

    const message = fillTemplate(template, { ...row, name: resolvedName });
    ...
    // نفس مبدأ campaignRunner.js: أي فشل تسجيل بعد القرار (sendLog/Registry/اسم جهة الاتصال)
    // متعملش كراش لكل الحملة - بيتلقط ويكمل للصف التالي
    try {
      switch (result.status) {
        ...
        case "sent": {
          ...
          summary.sent++;
          const meta = resolveContactMeta({ campaignType });
          const registryEntry = farmerRegistry.recordSuccessfulContact({
            ...
            nameArabic: usesName ? resolvedName : null,
          });
          await trySetContactName(client, phone, registryEntry.nameArabic);
          // Hook اختياري بعد إرسال ناجح فعلي (زي تفعيل حالة انتظار رد "استلام/توصيل") - مفيش تأثير
          // على مسار الإرسال نفسه، وأي خطأ فيه بيتلقط في الـcatch الشامل تحت زي أي فشل تسجيل تاني
          if (typeof onSent === "function") onSent({ phone: normalizeSaudiPhone(phone), name: usesName ? resolvedName : row.name, campaignType });
          break;
        }
        // الإرسال نجح فعليًا لواتساب، لكن الحفظ بعده فشل - ممنوع نعتبره فشل عادي (كان بيسمح
        // بإعادة إرسال حقيقي مكرر لنفس المزارع). محتاج مراجعة يدوية، مش إعادة محاولة تلقائية
        case "delivery_uncertain":
          console.log(`⚠️ الإرسال نجح فعليًا لكن الحفظ فشل بعده (${result.reason}) - DELIVERY_UNCERTAIN`);
          log.push(`${label} (${phone}) -> DELIVERY_UNCERTAIN (${result.reason})`);
          sendLog.logSend(logLabel, label, phone, "delivery_uncertain");
      ...
      console.log(`🚨 فشل تسجيل نتيجة هذا الصف (${persistErr.message}) - هنكمل باقي الحملة عادي`);
    }
